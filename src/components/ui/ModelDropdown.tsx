        ...
        <div className="relative flex">
          <ul className="flex-1 py-2" role="listbox" aria-label="Model options">
            {availableModels.map((model) => {
              ...
              const tooltipId = `${dropdownId}-tooltip-${model.id}`;
              const srDescriptionId = `${dropdownId}-sr-${model.id}`;
                ...
                <li key={model.id} className="relative">
                  ...
                  >
                    ...
                    <span id={srDescriptionId} className="sr-only">
                      {model.title}. {model.tooltipDescription || model.description}. Input
                      cost {model.pricing.input}. Output cost {model.pricing.output}
                      {model.contextWindow
                  ...
                  {showTooltip && (
                    <div
                      role="tooltip"
                      id={tooltipId}
                      className="pointer-events-none absolute left-full top-1/2 ml-4 w-64 -translate-y-1/2 rounded-md border border-gray-200 bg-white p-3 text-left shadow-xl"
                    >
